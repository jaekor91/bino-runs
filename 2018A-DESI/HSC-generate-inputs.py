...
for i in range(1, 7):
    ...
    # Data
    data = np.load("./data/HSC-%d-union-tractor.npz" % i)
    ra = data["ra"]
    dec = data["dec"] 
    ...
    data = np.load("./data/HSC-%d-sdss-stars.npz"%i)
    ra_star, dec_star, g_star = data["ra"], data["dec"], data["g"]

    data = np.load("./data/HSC-%d-sdss-gals.npz"%i)
    ...
    f = open("./input-catalogs/HSC-%d-inputs.txt" % i, "w")
    ...
    plt.savefig("./figures/RADEC-HSC-%d-inputs.png" % i, dpi=200, bbox_inches="tight")
    # # plt.show()
    plt.close()

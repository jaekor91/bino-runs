...
for i in range(1, 7):
    ...
    ax.axis("equal")
    plt.savefig(fig_dir+"RADEC-DR6-%d-union.png" % i, dpi=400, bbox_inches="tight")
    # plt.show()
    plt.close()
    ...
    ax.axis("equal")
    plt.savefig(fig_dir+"RADEC-DR6-%d-union-MC-unmatched.png" % i, dpi=400, bbox_inches="tight")
    # plt.show()
    plt.close()

    A = f_matched * np.cos(np.pi * dec_med / 180.) * (ra_max - ra_min) * (dec_max - dec_min) 

    np.savez("./data/DR6-%d-union-tractor.npz" % i, ra=ra, dec=dec, gflux=gflux, rflux=rflux, zflux=zflux, A=A)
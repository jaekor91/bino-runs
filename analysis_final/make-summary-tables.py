...
for m in range(len(save_dirs)):
    ...
    # --------------- Tally table, DENSE
    output = open(save_dir + "table-dense.txt", "w")
    ...
    # --------------- Tally table, light
    output = open(save_dir + "table-light.txt", "w")
    output.write("Tally by mask-selection\n")
    # Header string
    header_str = "%-15s\t"  % ("Mask Name")
    output.write("-" * 75 + "\n")
    ...
    # --------------- Tally table, DENSE without RF and Latex readable
    output = open(save_dir + "table-dense-noRF.txt", "w")
    # output.write("Tally by mask-selection\n")
    # Header string
    header_str = "%-15s &"  % ("Mask Name")
    # output.write("-" * 165 + "\n")
    ...
    for i in range(0, 16, 1):
        ...
        if (ibool.sum() > 0):
            # Iteration through the following bits
            print_str = "%-15s &" % (MASK_NAMES[i])
            for j, bit in enumerate(BIT_CODES[:4]):
                iselect = (np.bitwise_and(2**bit, BIT) > 0) & ibool
                Nsample = iselect.sum() # Number of samples in the selection
                Nz = ((REDZ[iselect] > 0.6) & (REDZ[iselect] < 1.6)).sum() # Number with redshift
                if Nsample == 0:

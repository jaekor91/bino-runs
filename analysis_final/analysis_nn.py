...
for q in range(1, len(dirs)//2):
	...
	for specnum in range(start_num, Nobjs):
		start = time.time()
		...
		if (not bit_true(bit, 2**1)) and (not bit_true(bit, 2**2)): # If the object is a target
			...
			zs, num_hits, cluster_sizes = z_candidates(wavegrid1, wavegrid2, detection1, detection2, \
			...
			if zs.size == 0: 
				...
				if zs.size > 50: # If there are more than 100 candidates
					print("More than 50 candidates so trimming to top 100 by num_hits and size cluster")
					# Create a sortable list of tuples
					a = [(zs[i], num_hits[i], cluster_sizes[i]) for i in range(zs.size)]
					a = sorted(a, key = lambda x: (x[1], x[2]))[::-1]
					for i in range(zs.size):
						z, hit, size = a[i]
						zs[i], num_hits[i], cluster_sizes[i] = z, hit, size
					zs = zs[:50]
					num_hits[:50]
